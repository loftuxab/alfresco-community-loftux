 ...
 * 
 * Generally remote URLs will be "data" webscripts (i.e. returning XML/JSON)
 * called from web-tier script objects and will be housed within an Alfresco
 * Repository server.
 * 
 * Also supports POST of content data
 * 
 * A 'Response' is returned containing the response data stream as a String and
 * the Status object representing the status code and error information if any.
 * 
 * @author Kevin Roast
 */
public class RemoteClient extends AbstractClient
{
     ...
     * Construction
     * 
     * @param endpoint HTTP API endpoint of remote Alfresco server webapp For
     *            example http://servername:8080/alfresco
     */
     ...
     * @param endpoint HTTP API endpoint of remote Alfresco server webapp For
     *            example http://servername:8080/alfresco
     * @param defaultEncoding Encoding to use when converting responses that do
     *            not specify one
     */
    ...
    /**
     * @param requestContentType the POST request "Content-Type" header value to
     *            set
     */
    ...
    /**
     * @param requestMethod the request Method to set i.e. one of
     *            GET/POST/PUT/DELETE etc. if not set, GET will be assumed
     *            unless an InputStream is supplied during call() in which case
     *            POST will always be used.
     */
     ...
     * 
     * @param uri WebScript URI - for example /test/myscript?arg=value
     * @param in The optional InputStream to the call - if supplied a POST will
     *            be performed
     * 
     * @return Response object from the call {@link Response}
     */
     ...
     * 
     * @param uri WebScript URI - for example /test/myscript?arg=value
     * @param buildResponseString True to build a String result automatically
     *            based on the response encoding, false to instead return the
     *            InputStream in the Response.
     * @param in The optional InputStream to the call - if supplied a POST will
     *            be performed
     * 
     * @return Response object from the call {@link Response}
    ...
    public Response call(String uri, boolean buildResponseString, InputStream in)
    {
        ...
        try
        {
            ...
            {
                result = new Response(new ByteArrayInputStream(
                        bOut.toByteArray()), status);
            }
     ...
     * 
     * @param uri WebScript URI - for example /test/myscript?arg=value
     * @param out OutputStream to stream successful response to - will be closed
     *            automatically. A response data string will not therefore be
     *            available in the Response object. If remote call fails the
     *            OutputStream will not be modified or closed.
     * 
     * @return Response object from the call {@link Response}
     */
    ...
    public Response call(String uri, InputStream in, HttpServletResponse res)
    {
        ...
        try
        {
            String encoding = service(buildURL(uri), in, res.getOutputStream(),
                    res, status);
            result = new Response(status);
            result.setEncoding(encoding);
        }
     ...
     * Ticket will be appiled as an argument if available.
     * 
    ...
    /**
     * Service a remote URL and write the the result into an output stream. If
     * an InputStream is provided then a POST will be performed with the content
     ...
     * @param in The optional InputStream - if set a POST will be performed
     * @param out The OutputStream to write result to
     * @param res Optional HttpServletResponse - to which response headers will
     *            be copied - i.e. proxied
     * @param status The status object to apply the response code too
     * 
     * @return encoding specified by the source URL - may be null
     * 
     ...
     */
    private String service(URL url, InputStream in, OutputStream out,
            HttpServletResponse res, Status status) throws IOException
    {
        if (logger.isDebugEnabled())
            logger.debug("Executing " + (in == null ? "(" + (requestMethod != null ? requestMethod : "GET") + ")" : "(POST)") + ' ' + url.toString());
        ...
        try
        {
            ...
            if (in != null)
            {
                ...
                FileCopyUtils.copy(in, new BufferedOutputStream(
                        connection.getOutputStream()));
            }
            // set the request method - if not supplied then the default of GET
            // will be used anyway
            ...
            finally
            {
                ...
                catch (IOException e)
                {
                    if (logger.isWarnEnabled())
                        logger.warn(
                                "Exception during close() of HTTP API connection",
                                e);
                }
            }
