 ...
 * credentials.
 * 
 * Authenticator objects are used when a "token" must be passed to the
 * endpoint and the current token is either invalid or non-existent.
 * The Connectors must then handshake with the endpoint to acquire
 * a token.
 * 
 * Tokens are not always necessary.  An example is HTTP Basic Authentication
 * where user names and passwords are sent on every request.  Alternatively,
 * of course, you may wish only to authenticate on the first request and
 * then pass the Authenticate hash on every subsequent request.
 * 
