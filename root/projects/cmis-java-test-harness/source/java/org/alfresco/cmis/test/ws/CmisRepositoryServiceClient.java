...
public class CmisRepositoryServiceClient extends AbstractServiceClient
{
    ...
    public void testGetRepositoryInfo()
    {
        ...
        assertEquals(getAndAssertRepositoryId(), repositoryInfo.getRepositoryId());
        assertNotNull(repositoryInfo.getRepositoryName());
        assertNotNull(repositoryInfo.getRepositoryRelationship());
        assertNotNull(repositoryInfo.getRepositoryDescription());
        assertNotNull(repositoryInfo.getProductName());
        assertNotNull(repositoryInfo.getVendorName());
        assertNotNull(repositoryInfo.getProductVersion());
        assertNotNull(repositoryInfo.getRootFolderId());
        assertFalse("".equals(repositoryInfo.getRootFolderId()));
        // FIXME: uncomment this when changeToken concept will be resolved
        // assertNotNull(repositoryInfo.getLatestChangeToken());
        assertNotNull(repositoryInfo.getCmisVersionSupported());
        assertNotNull(repositoryInfo.getThinClientURI());
        ...
        assertNotNull(capabilities.getCapabilityACL());
        assertNotNull(capabilities.getCapabilityChanges());
        ...
        assertNotNull(capabilities.getCapabilityContentStreamUpdatability());
        assertNotNull(capabilities.getCapabilityJoin());
        assertNotNull(capabilities.getCapabilityQuery());
        assertNotNull(capabilities.getCapabilityRenditions());
    }
    ...
    private GetTypeChildrenResponse getAndAssertTypeChildren(String typeId, boolean includeProperties, long maxItems, long skipCount)
    {
    ...
    public void testGetTypeChildrenWithTypeIdInNotSetAndSetState()
    {
        ...
        assertEquals(4, response.getType().length);
    ...
    private String getBaseDocumentTypeId(GetTypeChildrenResponse response)
    {
        String typeId = null;
        for (CmisTypeDefinitionType typeDef : response.getType())
        {
    ...
    public void testGetTypeChildrenPagination()
    {
        ...
        assertNotNull(response.isHasMoreItems());
        assertTrue(response.isHasMoreItems() && (3 == response.getType().length));

        response = getAndAssertTypeChildren(null, false, 0, 0);
        assertNotNull(response.isHasMoreItems());
        assertTrue(!response.isHasMoreItems() && (4 == response.getType().length));
        String typeId = null;
        for (CmisTypeDefinitionType typeDef : response.getType())
        {
        ...
        assertNotNull(response.isHasMoreItems());
        assertTrue(response.isHasMoreItems() && (2 == response.getType().length));

        response = getAndAssertTypeChildren(typeId, false, 10, 0);
        assertNotNull(response.isHasMoreItems());
        assertTrue((response.isHasMoreItems() && (10 == response.getType().length)) || (!response.isHasMoreItems() && (response.getType().length <= 10)));
    }
    ...
    public void testGetTypeChildrenWrongTypeId()
    {
        ...
        {
            assertTrue(e instanceof CmisFaultType && ((CmisFaultType) e).getType().equals(EnumServiceException.invalidArgument));
        }
    }
    ...
    private CmisTypeContainer[] getAndAssertTypeDescendants(String typeId, long depth, boolean typeDefinitions) throws RemoteException, CmisFaultType, ServiceException
    {
    ...
    public void testGetTypeDefinition() throws Exception
    {
        ...
        try
        {
            response = repositoryService.getTypeDefinition(new GetTypeDefinition(getAndAssertRepositoryId(), getTypesResponse[0].getType().getId()));
        }
        catch (Exception e)
        {
            fail(e.getMessage());
