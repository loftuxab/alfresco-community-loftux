...
public class BlogComponent extends AbstractUtils {
	...
	public void dataPrep_AONE() throws Exception {
		...
		SiteDashboardPage siteDashBoard = ShareUserDashboard.addDashlet(drone,
				siteName, Dashlets.WEB_QUICK_START);
		SiteWebQuickStartDashlet wqsDashlet = siteDashBoard.getDashlet(
				SITE_WEB_QUICK_START_DASHLET).render();
		wqsDashlet.selectWebsiteDataOption(WebQuickStartOptions.FINANCE);
		wqsDashlet.clickImportButtton();

		// Change property for quick start to sitename
		DocumentLibraryPage documentLibPage = ShareUser
		...
		EditDocumentPropertiesPage documentPropertiesPage = documentLibPage
				.getFileDirectoryInfo("Quick Start Editorial")
		...
		documentLibPage.getFileDirectoryInfo("Quick Start Live")
				.selectEditProperties().render();
		...
		ShareUserDashboard.addPageToSite(drone, siteName,
		...
		String setHostAddress = "cmd.exe /c echo. >> %WINDIR%\\System32\\Drivers\\Etc\\hosts && echo "
				+ ipAddress
				+ " "
				+ siteName
	...
	public void AONE_5675() throws Exception {
		...
		assertThat("Verify if the correct number of blog pages is displayed ",
				blogPage.getBlogPosts(), is(equalTo(3)));
	...
	public void AONE_5676() throws Exception {
		...
		assertThat("Posting was succesfull",
		...
		assertThat(wcmqsComment.getCommentFromContent(),
				is(equalTo(visitorComment)));
		...
		ShareUser.login(drone, ADMIN_USERNAME, ADMIN_PASSWORD);
		DocumentLibraryPage docLibPage = ShareUser.openSitesDocumentLibrary(
				drone, siteName).render();
		...
		DataListPage dataListPage = docLibPage.getSiteNav()
				.selectDataListPage().render();
		...
		feedbackList.render();
		VisitorFeedbackRow newFeedback = feedbackList.getRowForSpecificValues(
				visitorEmail, visitorComment, visitorName, visitorWebsite);
	...
	public void AONE_5677() throws Exception {
		...
		ShareUser.login(drone, ADMIN_USERNAME, ADMIN_PASSWORD);
		SiteDashboardPage shareSite = ShareUser.openSiteDashboard(drone,
				siteName);
		...
		DataListPage dataListPage = shareSite.getSiteNav().selectDataListPage()
				.render();
		...
		VisitorFeedbackRow testrow = feedbackList
				.getRowForVisitorEmail(visitorEmail);
		testrow.clickDuplicateOnRow();
		assertThat("Check if the duplicate message appears!",
				testrow.isDuplicateMessageDisplayed());
	...
	public void AONE_5678() throws Exception {
		...
		assertThat("Main form error message is displayed! ",
				wcmqsBlogPostPage.isFormProblemsMessageDisplay());
		assertThat(
				"Verify name field error is displayed",
				wcmqsBlogPostPage.getFormErrorMessages(),
		...
		assertThat("Main form error message is displayed! ",
				wcmqsBlogPostPage.isFormProblemsMessageDisplay());
		assertThat("Verify name field error is displayed",
				wcmqsBlogPostPage.getFormErrorMessages(),
	...
	public void AONE_5680() throws Exception {
		...
		wcmqsBlogPostPage.setVisitorComment(visitorComment);
		wcmqsBlogPostPage.clickPostButton();
		assertThat("Check if posting was succesfull",
				wcmqsBlogPostPage.isAddCommentMessageDisplay());
	...
	public void AONE_5681() throws Exception {
		...
		wcmqsBlogPostPage.clickPostButton();
		assertThat("Verify email field error is displayed",
				wcmqsBlogPostPage.getFormErrorMessages(),
		...
		wcmqsBlogPostPage.clickPostButton();
		assertThat("Verify comment textfield error is displayed",
				wcmqsBlogPostPage.getFormErrorMessages(),
	...
	public void AONE_5682() throws Exception {
		...
		wcmqsBlogPostPage.clickPostButton();
		assertThat("Verify invalid email error is displayed",
				wcmqsBlogPostPage.getFormErrorMessages(),
				hasItem(equalTo("the email address is not valid")));
	...
	public void AONE_5683() throws Exception {
		...
		navigateTo(getShareUrl());
		ShareUser.login(drone, ADMIN_USERNAME, ADMIN_PASSWORD);
		DataListPage dataListPage = ShareUser
				.openSiteDashboard(drone, siteName).render().getSiteNav()
				.selectDataListPage().render();
		dataListPage.selectDataList("Visitor Feedback (Quick Start Editorial)");
		VisitorFeedbackList feedbackList = new VisitorFeedbackList(drone);
		feedbackList.render();
		...
		VisitorFeedbackRowProperties visitorFeedbackRowProperties = new VisitorFeedbackRowProperties(
				drone).render();
		...
		wcmqsBlogPostPage = new WcmqsBlogPostPage(drone);
		assertThat("Verify if the new feedback comment has re-appeared",
				wcmqsBlogPostPage.getFeedBackComments(),
	...
	public void AONE_5684() throws Exception {
		...
		blogPage.clickLinkByTitle(WcmqsBlogPage.ETHICAL_FUNDS);
		wcmqsBlogPostPage = new WcmqsBlogPostPage(drone);
		assertThat("Leave comment form is displayed ",
				wcmqsBlogPostPage.isLeaveCommentFormDisplayed());
	}
	...
	public void AONE_5685() throws Exception {
		...
		assertThat(
				"Verify if tag list contains testtag 1 (2) and testtag 2 (1)",
				wcmqsBlogPage.getTagList(),
		...
		assertThat("Check if the number of results is correct", wcmqsSearchPage
				.getTagSearchResults().size(), is(equalTo(2)));
		...
		assertThat("Check if the number of results is correct", wcmqsSearchPage
				.getTagSearchResults().size(), is(equalTo(1)));
	}
	...
	public void AONE_5679() throws Exception {
		...
		wcmqsBlogPostPage.setVisitorName(visitorName
				+ StringUtils.leftPad("test", 1100, 'a'));
		assertThat("Check if the number of entered chars is 70",
				wcmqsBlogPostPage.getVisitorName().length(), is(equalTo(70)));
		...
		wcmqsBlogPostPage.setVisitorEmail(visitorEmail
				+ StringUtils.leftPad("test", 1100, 'a'));
		assertThat("Check if the number of entered chars is 100",
				wcmqsBlogPostPage.getVisitorEmail().length(), is(equalTo(100)));
		...
		wcmqsBlogPostPage.setVisitorWebsite(visitorWebsite
				+ StringUtils.leftPad("test", 1100, 'a'));
		assertThat("Check if the number of entered chars is 100",
				wcmqsBlogPostPage.getVisitorWebsite().length(),
				is(equalTo(100)));
