...
{	
	...
	if (!this.minimizeFx)
	{
		this.minimizeFx = new Fx.Styles(this.generalLayer, {
			duration: 300,
	...
	if (!this.maximizeFx)
	{
		this.maximizeFx = new Fx.Styles(this.generalLayer, {
			duration: 300,
			transition: Fx.Transitions.linear,
...
WebStudio.Window.prototype.setActive = function()
{
...
WebStudio.Window.prototype.setElementConfig = function(item, index, ob, config, oel) 
{
	...
	if (config.dragWindow) 
	{
		...
		this.moveObjects[this.moveObjectsID] = {};
	...
	{
		this.reCalcPositionObjects[this.reCalcPositionObjectsID] = {};
		ro = this.reCalcPositionObjects[this.reCalcPositionObjectsID];
		ro.index = index;
		ro.object = ob;
		ro.elObject = oel;
	...
	{
		this.resetStylesObjects[this.resetStylesObjectsID] = {};
		rs = this.resetStylesObjects[this.resetStylesObjectsID];
		rs.index = index;
		rs.object = ob;
		rs.elObject = oel;
		rs.type = 'windowHide';
		...
		if (config.onHide)
		{
			rs.onHide = config.onHide;
		}
		if (config.onShow)
		{
			rs.onShow = config.onShow;
		}
...
WebStudio.Window.prototype.resizeElements = function() 
{
	...
	$each(this.reCalcPositionObjects, function(item, index) {
		...
		if (item.type == 'right') 
		{
			item.elObject.setStyle('left', _this.generalLayer.getStyle('width').toInt() - item.offset);
		} 
		else if (item.type == 'center') 
		{
			w = Alf.parseInt((_this.generalLayer.getStyle('width').toInt() - item.elObject.getStyle('width').toInt()) / 2);
			item.elObject.setStyle('left', w);
		} 
		else if (item.type == 'bottom') 
		{
			item.elObject.setStyle('top', _this.generalLayer.getStyle('height').toInt() - item.offset);
		} 
		else if (item.type == 'width') 
		{
			w = _this.generalLayer.getStyle('width').toInt() - item.offset;
			if (w < 0)
			{
				w = 0;
			}
		...
		{
			item.elObject.setStyle('height', _this.generalLayer.getStyle('height').toInt() - item.offset);
		}
	});
};
...
WebStudio.Window.prototype.resetStyles = function() 
{
	...
	$each(this.resetStylesObjects, function(item, index) {
		...
		{
			if ((_this.generalLayer.getStyle('width').toInt() <= item.width)&& !item.isHide ) 
			{
			...
			else if ((_this.generalLayer.getStyle('width').toInt() > item.width)&& item.isHide ) 
			{
		...
		events: {
			mousedown: function(event) {
				...
				if (document.selection)
				{
					...
					{
						//Clear current selection for IE
				...
				sw.mouseMoveListener = function(event) {
					...
					{
						pw = Alf.parseInt(this.mdh) + event.clientY - this.mdy;
					...
					else if (type == "n-resize") 
					{
						...
						{
							this.generalLayer.style.height = pw + 'px';
							this.generalLayer.style.top = Alf.parseInt(this.mdt) + (event.clientY - this.mdy) + 'px';
						} 
						else if (pw > this.maxHeight) 
						{
							this.generalLayer.style.height = this.maxHeight + 'px';
							this.generalLayer.style.top = Alf.parseInt(this.mdt) + (Alf.parseInt(this.mdh) - this.maxHeight) + 'px';
						} 
						else if (pw < this.minHeight) 
						{
							this.generalLayer.style.height = this.minHeight + 'px';
							this.generalLayer.style.top = Alf.parseInt(this.mdt) + (Alf.parseInt(this.mdh) - this.minHeight) + 'px';
						}
					} 
					else if (type == "w-resize") 
					{
						...
						{
							this.generalLayer.style.width = pw + 'px';
							this.generalLayer.style.left = Alf.parseInt(this.mdl) + (event.clientX - this.mdx) + 'px';
						}
						else if (pw > this.maxWidth) 
						{
							this.generalLayer.style.width = this.maxWidth + 'px';
							this.generalLayer.style.left = Alf.parseInt(this.mdl) + (Alf.parseInt(this.mdw) - this.maxWidth) + 'px';
						}
						else if (pw < this.minWidth) 
						{
							this.generalLayer.style.width = this.minWidth + 'px';
							this.generalLayer.style.left = Alf.parseInt(this.mdl) + (Alf.parseInt(this.mdw) - this.minWidth) + 'px';
						}
					} 
