			...
			events: {
				over: function(el, obj) {
					...
					if (slider)
					{
						if (slider.Dropper)
						{
							if (slider.Dropper.el)
							{
								slider.Dropper.el.setStyle('display', 'block');
							}
						}
					}
				},
				leave: function(el, obj) {
					...
					if (slider)
					{
						if (slider.Dropper)
						{
							if (slider.Dropper.el)
							{
								slider.Dropper.el.setStyle('display', 'none');
							}
						}
					}
				},
...
WebStudio.SlidersSector.prototype = {
	...
	showSliderData: function(index, h) {
		if (this.SlidersData[index])
		{
			if (!h)
			{
				h = this.getHeight().toInt() - this.getAllSlidersHeight().toInt() - 27;
			}
	...
	hideAllSlidersData: function() {
		...
		$each(this.Sliders, function(item, index) {
			_this.SlidersData[index].el.setStyle('height', 0);
			_this.SlidersData[index].state = 'hide';
			_this.SliderTogglers[index].el.removeClass('Show');
		});
		this.activeSliderIndex = false;
	},
	...
	getAllSlidersHeight: function() {
		...
		this._tempH = 0;
		$each(this.Sliders, function(item, index) {
			_this._tempH += item.el.getSize().size.y;
		});
	...
	applyElementsConfig: function() {
		...
		$each(this.elementsConfig, function(cItem, cIndex) {
			...
			if (cItem.selector)
			{
				...
				_this[cIndex].els = els;
				$each(els, function(elItem, elIndex) {
					_this.setElementProperties(elItem, elIndex, cItem, cIndex, _this);
				});
			}
		});
		return this;
	...
	setElementProperties: function(item, index, config, parentIndex, parent) { //'item' = html dom element, 'index' = name of array of 'item's; ob = js container for 'index'; config = js object with configuration for item
		...
		if (o.el) 
		{
			if (config.styles)
			{
				o.el.setStyles(config.styles);
			}
			
			if (config.topMultiplier)
			{
				o.el.set({
					styles: {
			...
			if (config.objects) 
			{
				...
				$each(config.objects, function(cItem, cIndex) {
					...
					if (cItem.selector) 
					{
						...
						o[cIndex].els = els;
						$each(els, function(elItem, elIndex) {
							_this.setElementProperties(elItem, elIndex, cItem, cIndex, o);
						});
					}
	...
	setDragDrop: function(el, index) {
		...
		el.addEvent('mousedown', function(e) {
			...
			clone.setStyles(this.getParent().getCoordinates());
			clone.setStyles({
				'opacity': 0.7, 
				'position': 'absolute',
                'left' : '0px'
			...
			clone.inject(WebStudio.App.generalLayer);
	...
	setTemplateBySelector: function(selector) { 
	
		//Set template of the window by selector(string)
	...
	addDebugMessages: function(type, text, more) { 
		...
		//Add debug message in dojo debug container or alert it
		if (this.isAlertDM)
		{
			alert(text);
		}
