if (typeof AAF == "undefined" || !AAF)
{
   AAF = function AAF()
   {
      var config = {};
      return {
         init: function AW_Init(aafConfig)
         {
            config = aafConfig;
            this.loader.init(config.loaderConfig);
            // if (!AAF.constants && config.constants)
            //            {
            //               AAF.constants = config.constants;
            //            }
         },
         boot: function AAF_Boot(callback)
         {
...
if (typeof AAF.loader == "undefined" || !AAF.loader)
{
	AAF.loader = (function() {
      var loadingModules = [],
          unloadedModules = [],
       ...
       *  
       */
      var init =  function AAF_initialise(config, callback){ 
       ...
       *  
       */
      var boot =  function AAF_initialise(callback){ 
         var loadYUILoader = function loadYUILoader(yuiloaderPath, callback)
         {
            ...
            node.id = 'aaf-yuiloader' + loadYUILoader._counter;
       ...
       */
      var convertConfigToYUIModuleConfig = function AAF_convertConfigToYUIModuleConfig(config)
      {
         var yuiConfig = {
            name: config.name,
            type: config.type || 'js',
         ...
         else
         {
            ...
            yuiConfig.varName = "AAF_Loader_Variable_" + n + convertConfigToYUIModuleConfig._counter;
         }
       ...
       * 
       */
      var require = function AAF_require(config, isYUILoaderCompatible)
      {
       ...
       */
      var load = function AAF_load(successCallback, failureCallback)
      {
         var loaderConfig = {};
         ...
         {
            var yuiloader_fail = function AAF_YUILoader_failure(msg, obj)
            {
               if (failureCallback)
               {
                  failureCallback.fn.call(failureCallback.scope||window,msg, obj);
               }
               throw new Error('AAF_YUI_Loader failure');
            };
         ...
         {          
            var yuiloader_success = function AAF_YUILoader_success(msg, obj)
            {
               for (var i = 0,len = loadingModules.length;i<len;i++)
               {
