...
public abstract class HostAnnouncer extends Thread {
  ...
  public void run() {
      ...
      try {
        ...
        if ( isNetworkEnabled()) {
		        ...
		        if ( Debug.EnableError && hasDebug())
				  ...
				  //	Reset the sleep interval to the starting interval as the network connection is not
				  //	available
        ...
        if ( isNetworkEnabled() && sleepTime < sleepNormal) {
          ...
          //	Double the sleep interval until it exceeds the configured announcement interval.
    ...
    //  Set the announcement interval to zero to indicate that the host is leaving Network Neighborhood
    ...
    //  Send out a number of host announcement to remove the host name(s) from Network Neighborhood
