...
/*
 * Win32NetBIOSPacketHandler.java
 *
 ...
 * 
 * <p>Uses the Win32 Netbios() call to provide the low level session layer for better integration with Windows.
 */
  ...
  public int readPacket(SMBSrvPacket pkt)
    ...
    if (( rxLen & ReceiveErrorMask) != 0) {
      ...
      if ( sts == NetBIOS.NRC_Incomp) {
        ...
        //	Check if the packet buffer is already at the maximum size (we assume the maximum size is the maximum
...
//          Debug.println("readPacket() extended buffer to " + pkt.getBuffer().length);
        ...
        //
        //	Note:	If the second read request is issued with a size of 64K or 64K-4 it returns with another
...
//        Debug.println("readPacket() rxlen2=" + rxLen2 + ", total read len = " + (rxLen + rxLen2));
  ...
  public void writePacket(SMBSrvPacket pkt, int len)
    ...
    //
    //	As Windows is handling the NetBIOS session layer we do not send the 4 byte header that is used
