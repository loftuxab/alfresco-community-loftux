...
import org.alfresco.jlan.server.filesys.FilesystemPendingException;
 ...
 * 
 * <p>The NT protocol handler processes the additional SMBs that were added to the protocol
 * in the NT SMB dialect.
 */
public class NTProtocolHandler extends CoreProtocolHandler {
  ...
  public boolean runProtocol()
    ...
    //  Determine if the request has a chained command, if so then we will copy the incoming request so that
    ...
    if (chainedCmd) {
      ...
      outPkt = new SMBSrvPacket(m_smbPkt, m_smbPkt.getPacketLength());
		...
		//	Set the process id from the received packet, this can change for the same session and needs to be set
  ...
  protected final int procChainedTreeConnectAndX(int cmdOff, SMBSrvPacket outPkt, int endOff) {
    ...
    if ( m_sess.hasClientInformation() && m_sess.getClientInformation().isNullSession() && servType != ShareType.ADMINPIPE) {
      ...
      //	Return an error status
   		...
   		outPkt.setError(m_smbPkt.isLongErrorCode(), SMBStatus.NTAccessDenied, SMBStatus.DOSAccessDenied, SMBStatus.ErrDos);
      return endOff;
    }
    ...
    try {
    	...
    	//	Return a logon failure status
			...
			outPkt.setError(m_smbPkt.isLongErrorCode(), SMBStatus.NTLogonFailure, SMBStatus.DOSAccessDenied, SMBStatus.ErrDos);
			return endOff;
    }
    catch ( Exception ex) {
    	...
    	//	Return a general status, bad network name
			...
			outPkt.setError(m_smbPkt.isLongErrorCode(), SMBStatus.NTBadNetName, SMBStatus.SRVInvalidNetworkName, SMBStatus.ErrSrv);
			return endOff;
    }
    ...
    if (auth != null && auth.getAccessMode() == CifsAuthenticator.SHARE_MODE) {
      ...
      if (sharePerm < 0) {
		...
		//	Check if there is an access control manager, if so then run any access controls to determine the
		...
		if ( getSession().getServer().hasAccessControlManager() && shareDev.hasAccessControls()) {
			...
			//	Update the access permission for this session by processing the access control list for the
			//	shared device
  ...
  protected final int procChainedReadAndX(int cmdOff, SMBSrvPacket outPkt, int endOff, NetworkFile netFile) {
    ...
    long offset = (long) m_smbPkt.getAndXParameterLong(cmdOff, 3);		//	bottom 32bits of read offset
    offset &= 0xFFFFFFFFL;
    ...
    try {
      ...
      outPkt.setAndXParameter(endOff, 6, dataPos - RFCNetBIOSProtocol.HEADER_LEN);    // offset to data
  ...
  protected final int procChainedClose(int cmdOff, SMBSrvPacket outPkt, int endOff) {
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_FILE))
      m_sess.debugPrintln("Chained File Close [" + m_smbPkt.getTreeId() + "] fid=" + fid);
  ...
  protected void procTreeConnectAndX(SMBSrvPacket outPkt)
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_TREE))
      m_sess.debugPrintln("Tree Connect AndX - Allocated Tree Id = " + treeId + ", Permission = " + FileAccess.asString(sharePerm));
  ...
  protected void procCloseFile(SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException {
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_FILE))
      m_sess.debugPrintln("File close [" + m_smbPkt.getTreeId() + "] fid=" + fid);
  ...
  protected void procTransact2(SMBSrvPacket outPkt)
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_TRAN))
      m_sess.debugPrintln("Transaction [" + m_smbPkt.getTreeId() + "] tbuf=" + transBuf);
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_LOCK))
      m_sess.debugPrintln("File Lock [" + netFile.getFileId() + "] : type=0x" + Integer.toHexString(lockType) + ", tmo="
		...
		if ( disk instanceof FileLockingInterface) {
			...
			//	Return a 'not locked' status if there are unlocks in the request else return a success status
  ...
  protected final void procOpenAndX(SMBSrvPacket outPkt)
    ...
    try {
      ...
      return;
			...
			m_sess.sendErrorResponseSMB(SMBStatus.NTSharingViolation, SMBStatus.DOSFileSharingConflict, SMBStatus.ErrDos);
			return;
    }
      ...
      m_sess.sendErrorResponseSMB( SMBStatus.NTObjectPathNotFound, SMBStatus.HRDDriveNotReady, SMBStatus.ErrHrd);
      return;
    }
    catch (java.io.IOException ex) {
  ...
  protected final void procReadAndX(SMBSrvPacket outPkt)
    ...
    int fid = m_smbPkt.getParameter(2);
    long offset = m_smbPkt.getParameterLong(3);		//	bottom 32bits of read offset
    offset &= 0xFFFFFFFFL;
    int maxCount = m_smbPkt.getParameter(5);
		...
		if ( m_smbPkt.getParameterCount() == 12) {
			long topOff = m_smbPkt.getParameterLong(10);
			offset += topOff << 32;
		}
    ...
    catch (FilesystemPendingException ex) {
      
      //  Debug

      if (Debug.EnableError && m_sess.hasDebug(SMBSrvSession.DBG_FILEIO))
        m_sess.debugPrintln("File Read Pending [" + netFile.getFileId() + "] : " + ex.toString());
        
      //	Filesystem driver is still processing the read request
      //
      //	Clone a new SMB packet and continue processing new requests.
      
      m_smbPkt = new SMBSrvPacket( m_smbPkt);
      m_sess.setReceivePacket( m_smbPkt);
      
      //	The read response will be sent asynchronously by the filesystem driver.
  ...
  protected void procRenameFile(SMBSrvPacket outPkt)
    ...
    try {
      ...
      m_sess.sendErrorResponseSMB(SMBStatus.NTObjectNameCollision, SMBStatus.DOSFileAlreadyExists, SMBStatus.ErrDos);
      return;
    }
    catch ( AccessDeniedException ex) {
    ...
    try {
      ...
      if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_SEARCH))
        m_sess.debugPrintln("Start trans search [" + searchId + "] - " + srchPath + ", attr=0x" + Integer.toHexString(srchAttr)
            + ", maxFiles=" + maxFiles  + ", maxLen=" + maxLen + ", infoLevel="  + infoLevl + ", flags=0x" + Integer.toHexString(srchFlag));
      ...
      //  Pack the parameter block
			...
			if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_SEARCH))
				m_sess.debugPrintln("Search [" + searchId + "] Returned " + fileCnt + " files, dataLen=" + dataBuf.getLength() + ", moreFiles=" + ctx.hasMoreFiles());
  ...
  protected final void procTrans2FindNext(SrvTransactBuffer tbuf, SMBSrvPacket outPkt)
    ...
    try {
      ...
      if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_SEARCH))
        m_sess.debugPrintln("Continue search [" + searchId + "] - " + resumeName + ", maxFiles=" + maxFiles + ", maxLen=" + maxLen +
    ...
    try {
      ...
      switch (infoLevl) {
        ...
        case DiskInfoPacker.InfoMacFsInfo:
          ...
          //
          //	We should only return a valid response to the Macintosh information level if the filesystem
          //	does NOT support NTFS streams. By returning an error status the Thursby DAVE software will treat
        	...
        	if ( ntfs == false) {
         	...
         	break;
				...
				case DiskInfoPacker.InfoFullFsSize:
					...
					//	Check if there is a quota manager configured, if so then get the per user free space
					...
					//	If the per user free space is not valid then use the total available free space, else convert
					//	to allocation units.
  ...
  protected final void procTrans2QueryPath(SrvTransactBuffer tbuf, SMBSrvPacket outPkt)
    ...
    if (m_sess.hasDebug(SMBSrvSession.DBG_INFO))
  ...
  protected final void procTrans2QueryFile(SrvTransactBuffer tbuf, SMBSrvPacket outPkt)
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
      m_sess.debugPrintln("Query File - level=0x" + Integer.toHexString(infoLevl) + ", fid=" + fid + ", stream=" + netFile.getStreamId() + ", name=" + netFile.getFullName());
  ...
  protected final void procTrans2SetFile(SrvTransactBuffer tbuf, SMBSrvPacket outPkt)
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
      m_sess.debugPrintln("Set File - level=0x" + Integer.toHexString(infoLevl) + ", fid=" + fid + ", name=" + netFile.getFullName());
      ...
      DiskInterface disk = (DiskInterface) conn.getSharedDevice().getInterface();
			...
			switch ( infoLevl) {
		      ...
		      disk.setFileInformation(m_sess, conn, netFile.getFullName(), finfo);
			    ...
			    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
			      m_sess.debugPrintln("  Set Basic Info [" + treeId + "] name=" + netFile.getFullName() + ", attr=0x" + Integer.toHexString(attr) +
			          								", setTime=" + hasSetTime + ", setFlags=0x" + Integer.toHexString(setFlags) + ", unknown=" + unknown);
					break;
				...
				case FileInfoLevel.NTFileRenameInfo:
					...
					if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
						m_sess.debugPrintln("  Set rename fid=" + fid + ", newName=" + newName + ", overwrite=" + overwrite + ", rootFID=" + rootFid);
					...
					if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_STREAMS))
						m_sess.debugPrintln("Rename stream fid=" + fid + ", name=" + netFile.getFullNameStream() + ", newName=" + newName + ", overwrite=" + overwrite);
		      ...
		      //  Call the filesystem driver set file information to see if the file can be marked for
					//	delete.
		      ...
		      disk.setFileInformation(m_sess, conn, netFile.getFullName(), delInfo);
					...
					if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
						m_sess.debugPrintln("  Set file disposition fid=" + fid + ", name=" + netFile.getName() + ", delete=" + delFlag);
					break;
			}
	    ...
	    //	Check if there are any file/directory change notify requests active
			...
			if ( diskCtx.hasChangeHandler() && netFile.getFullName() != null) {
			  ...
			  if ( finfo != null) {
				...
				else if ( infoLevl == FileInfoLevel.SetAllocationInfo ||
									infoLevl == FileInfoLevel.SetEndOfFileInfo) {
      ...
      int attr = 0;
			...
			switch ( infoLevl) {
		      ...
		      disk.setFileInformation(m_sess, conn, path, finfo);
			    ...
			    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
			      m_sess.debugPrintln("  Set Standard Info [" + treeId + "] name=" + path + ", attr=0x" + Integer.toHexString(attr) +
			          								", setTime=" + hasSetTime + ", setFlags=0x" + Integer.toHexString(setFlags) + ", eaListLen=" + eaListLen);
					break;
        ...
        case FileInfoLevel.SetBasicInfo:
          ...
          if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
            m_sess.debugPrintln("  Set Basic Info [" + treeId + "] name=" + path + ", attr=0x" + Integer.toHexString(attr) +
                                ", setFlags=0x" + Integer.toHexString(setFlags));
          break;
      }
  ...
  protected final void procWriteAndX(SMBSrvPacket outPkt)
    ...
    int fid = m_smbPkt.getParameter(2);
    long offset = (long) (((long) m_smbPkt.getParameterLong(3)) & 0xFFFFFFFFL);		//	bottom 32bits of file offset
    ...
    try {
      ...
      return;
			...
			if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_FILEIO))
				m_sess.debugPrintln("Write Quota Error [" + netFile.getFileId() + "] Disk full : Size=" + dataLen + " ,Pos=" + offset);
    ...
    //	Report file size change notifications every so often
		...
		if ( netFile.getWriteCount() % FileSizeChangeRate == 0 &&
		     diskCtx.hasChangeHandler() && netFile.getFullName() != null) {
  ...
  protected final void procNTCreateAndX(SMBSrvPacket outPkt)
    ...
    String fileName = DataPacker.getUnicodeString(m_smbPkt.getBuffer(), DataPacker.wordAlign(m_smbPkt.getByteOffset()), nameLen/2);
    if (fileName == null) {
      m_sess.sendErrorResponseSMB(SMBStatus.NTInvalidParameter, SMBStatus.SRVNonSpecificError, SMBStatus.ErrSrv);
      return;
    }
		...
		//	Check if the file name contains a file stream name. If the disk interface does not implement the optional NTFS
    ...
    try {
      ...
      if (fileSts == FileStatus.NotExist) {
        ...
        if (( createOptn & WinNT.CreateNonDirectory) != 0 && fileSts == FileStatus.DirectoryExists) {
          ...
          m_sess.sendErrorResponseSMB( SMBStatus.NTFileIsADirectory, SMBStatus.DOSAccessDenied, SMBStatus.ErrDos);
          return;
        }
      ...
      m_sess.sendErrorResponseSMB(SMBStatus.NTObjectNameInvalid, SMBStatus.DOSInvalidFormat, SMBStatus.ErrDos);
      return;
    }
    catch ( DiskOfflineException ex) {
    ...
    outPkt.setParameter(1, 0); // AndX offset
		...
		prms.packWord((flags & WinNT.ExtendedResponse) != 0 ? 7 : 0);
                                                //	Device state
  ...
  protected final void procNTCancel(SMBSrvPacket outPkt)
    ...
    if (conn == null) {
      ...
      return;
		...
		NotifyRequest req = m_sess.findNotifyRequest(m_smbPkt.getMultiplexId(), m_smbPkt.getTreeId(), m_smbPkt.getUserId(),
	    ...
	    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_NOTIFY)) {
	    	DiskDeviceContext diskCtx = (DiskDeviceContext) conn.getContext();
	      m_sess.debugPrintln("NT Cancel notify mid=" + req.getMultiplexId() + ", dir=" + req.getWatchPath() + ", queue=" + diskCtx.getChangeHandler().getRequestQueueSize());
	    }
		}
		else {
  ...
  protected final void procNTTransaction(SMBSrvPacket outPkt)
    ...
    int subCmd = ntTrans.getNTFunction();
	    ...
	    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_TRAN))
	      m_sess.debugPrintln("NT Transaction [" + treeId + "] pcnt=" + ntTrans.getNTParameter(4) + ", offset=" + ntTrans.getNTParameter(5));
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_TRAN))
      m_sess.debugPrintln("NT Transaction [" + treeId + "] cmd=0x" + Integer.toHexString(subCmd) + ", multiPkt=" + transBuf.isMultiPacket());
  ...
  protected final void procNTTransactIOCtl(SrvTransactBuffer tbuf, NTTransPacket outPkt) 
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_TRAN))
      m_sess.debugPrintln("NT IOCtl code=" + NTIOCtl.asString(ctrlCode) + ", fid=" + fid + ", fsctrl=" + fsctrl + ", filter=" + filter);
  ...
  protected final void procNTTransactQuerySecurityDesc(SrvTransactBuffer tbuf, NTTransPacket outPkt) 
    ...
    if ( disk instanceof SecurityDescriptorInterface) {
      ...
      if ( tbuf.getReturnDataLimit() == 0) {
        ...
        //  Set a warning status to indicate the supplied data buffer was too small to return the security
        //  descriptor
        ...
        outPkt.initTransactReply(paramblk, paramblk.length, secBuf, secLen);
      }
    }
    else {
  		...
  		if ( tbuf.getReturnDataLimit() == 0) {
  			...
  			//	Initialize the transaction reply. Return the fixed security descriptor that allows anyone to access the
  			//	file/directory
  ...
  protected final void procNTTransactSetSecurityDesc(SrvTransactBuffer tbuf, NTTransPacket outPkt) 
    ...
    if ( disk instanceof SecurityDescriptorInterface) {
      ...
      outPkt.initTransactReply(null, 0, null, 0);
      outPkt.setError( SMBStatus.Success, SMBStatus.Success);
      m_sess.sendResponseSMB( outPkt);
    }
    else {
  ...
  protected final void procNTTransactNotifyChange(NTTransPacket ntpkt, SMBSrvPacket outPkt) 
    ...
    if (dir == null) {
      ...
      return;
		...
		//	Get the maximum notifications to buffer whilst waiting for the request to be reset after a notification
		//	has been triggered
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_NOTIFY))
      m_sess.debugPrintln("NT NotifyChange fid=" + fid + ", mid=" + mid + ", filter=0x" + Integer.toHexString(filter) + ", dir=" + dir.getFullName() + ", maxQueue=" + maxQueue);

		//	Check if there is an existing request in the notify list that matches the new request and is in a completed
		//	state. If so then the client is resetting the notify request so reuse the existing request.
		...
		if ( req != null && req.isCompleted()) {
			...
			if ( req.hasBufferedEvents() || req.hasNotifyEnum()) {
				...
				if ( Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_NOTIFY)) {
				  if ( bufList == null)
					  ...
					  m_sess.debugPrintln("   Sent buffered notifications, req=" + req.toString() + ", count=" + bufList.numberOfEvents());
				}
			}
			else {
			...
			req = new NotifyRequest(filter, watchTree, m_sess, dir, mid, ntpkt.getTreeId(),
															ntpkt.getProcessId(), ntpkt.getUserId(), maxQueue);
			...
			if ( Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_NOTIFY)) {
				...
				m_sess.debugPrintln("   Global notify mask = 0x" + Integer.toHexString(diskCtx.getChangeHandler().getGlobalNotifyMask()) +
														", reqQueue=" + diskCtx.getChangeHandler().getRequestQueueSize());
			}
		}
					
    //	NOTE:	If the change notification request is accepted then no reply is sent to the client. A reply will be sent
