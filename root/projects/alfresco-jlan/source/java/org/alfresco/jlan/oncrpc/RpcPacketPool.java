...
public class RpcPacketPool {
  ...
  public final RpcPacket allocatePacket ( int reqSize) {
    ...
    if ( reqSize <= m_smallPktSize) {
      ...
      if ( m_debug)
        Debug.println("RpcPacketPool Allocated (small) " + pkt.getBuffer() + ", len=" + pkt.getBuffer().length + ", list=" + m_smallPackets.size() + "/" + m_smallPktLimit);
    }
    else {
      ...
      if ( m_debug)
        Debug.println("RpcPacketPool Allocated (large) " + pkt.getBuffer() + ", len=" + pkt.getBuffer().length + ", list=" + m_largePackets.size() + "/" + m_largePktLimit);
    }
  ...
  public final void releasePacket(RpcPacket pkt) {
    ...
    if ( pkt.getBuffer().length >= m_largePktSize) {
      ...
      synchronized ( m_largePackets) {
        ...
        if ( m_debug)
          Debug.println("RpcPacketPool Released (large) " + pkt.getBuffer() + ", len=" + pkt.getBuffer().length + ", list=" + m_largePackets.size());
      }
    }
    else {
