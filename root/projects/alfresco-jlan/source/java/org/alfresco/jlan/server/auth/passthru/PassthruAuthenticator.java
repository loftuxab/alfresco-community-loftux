...
public class PassthruAuthenticator extends CifsAuthenticator implements SessionListener {
  ...
  public void processSessionSetup(SMBSrvSession sess, SMBSrvPacket reqPkt, SMBSrvPacket respPkt) throws SMBSrvException {
    ...
    try {
      ...
      if (isNTLMSSP == true) {
        ...
        if (hasDebug())
          Debug.println("NT Session setup NTLMSSP, MID=" + reqPkt.getMultiplexId() + ", UID=" + reqPkt.getUserId() + ", PID="
              + reqPkt.getProcessId());
    ...
    if (hasDebug() && sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE))
      Debug.println("User " + client.getUserName() + " logged on "
  ...
  private final byte[] doNtlmsspSessionSetup(SMBSrvSession sess, ClientInfo client, byte[] secbuf, int secpos, int seclen,
      boolean unicode) throws SMBSrvException {
