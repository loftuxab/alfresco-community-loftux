...
class CoreProtocolHandler extends ProtocolHandler {
  ...
  /**
   * Map a Java exception class to an SMB error code, and return an error response to the
   * caller.
   *
  ...
  protected final int packSearchInfo(byte[] buf, int bufPos, String searchStr, int resumeId, int searchId, FileInfo info) {
    ...
    //  Check if the string buffer has been allocated
    ...
    m_strBuf.append(info.getFileName());

    while (m_strBuf.length() < 13)
      m_strBuf.append('\0');

    if (m_strBuf.length() > 12)
      m_strBuf.setLength(12);

    DataPacker.putString(m_strBuf.toString().toUpperCase(), buf, bufPos, true);
    bufPos += 13;
  ...
  protected void procCheckDirectory(SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException {
    ...
    try {
      ...
      if (disk.fileExists(m_sess, conn, dirName) == FileStatus.DirectoryExists) {
        ...
        //
        //  DOS clients depend on the 'Directory Invalid' (SMB_ERR_BAD_PATH) message being returned.
  ...
  protected void procCreateDirectory(SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException {
    ...
    try {
      ...
      DiskInterface disk = (DiskInterface) conn.getSharedDevice().getInterface();
			...
			FileOpenParams params = new FileOpenParams(dirName, FileAction.CreateNotExist, AccessMode.ReadWrite, FileAttribute.NTDirectory);
  ...
  protected void procReadFile(SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException {
    ...
    try {
      ...
      DiskInterface disk = (DiskInterface) conn.getSharedDevice().getInterface();
	      ...
	      if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_FILEIO))
	        m_sess.debugPrintln("File Read [" + netFile.getFileId() + "] Limited to " + availCnt);
			}
  ...
  protected final void procSearch(SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException {
    ...
    //  Default resume point is at the start of the directory, at the '.' directory if directories are
    //  being returned.
    ...
    if (resumeLen == 0 && srchPath.length() > 0) {
      ...
      if (searchId == -1) {
        ...
        //
        //  Windows Explorer seems to leak searches after a new folder has been created, a search for '????????.???'
    ...
    if ((srchAttr & FileAttribute.Directory) != 0
      && resumeId >= RESUME_DOTDOT
      && WildCard.containsWildcards(srchPath)) {

      //  The first entries in the search should be the '.' and '..' entries for the current/parent
      //  directories.
      //
      //  Remove the file name from the search path, and get the file information for the search
      //  directory.
    ...
    if (fileCnt == 0) {
      ...
      //  Check if the search string contains wildcards and this is the start of a new search, if not then
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_TXDATA))
      m_sess.debugPrintln("Tx " + m_smbPkt.getLength() + " bytes");
  }
...
**/
  ...
  protected void procSetFileAttributes(SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException {
    ...
    if (Debug.EnableInfo && m_sess.hasDebug(SMBSrvSession.DBG_FILE))
      m_sess.debugPrintln("Set File Attributes [" + treeId + "] name=" + fileName + ", attr=0x" + Integer.toHexString(fattr) + ", fdate=" + fdate + ", ftime=" + ftime);
  ...
  protected void procTreeConnect(SMBSrvPacket outPkt)
  ...
  protected void procWriteFile(SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException {
    ...
    try {
      ...
      pos += 3;
			...
			//	Check for a zero length write, this should truncate/extend the file to the write offset position
    ...
    switch (m_smbPkt.getCommand()) {
      ...
      default :
        ...
        //  Indicate that the protocol handler did not process the SMB request
        	...
        	Debug.println("<<<<< Unknown SMB type : 0x" + Integer.toHexString(m_smbPkt.getCommand()) + " >>>>>");
        handledOK = false;
        break;
    }
