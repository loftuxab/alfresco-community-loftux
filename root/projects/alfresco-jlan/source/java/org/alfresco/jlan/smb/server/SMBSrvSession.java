...
/**
 * <p>The SMB server creates a server session object for each incoming session request.
 *
 * <p>The server session holds the context of a particular session, including the list of open
 * files and active searches.
 */
public class SMBSrvSession extends SrvSession implements Runnable {
  ...
  public static final int MaxConnections 		= 16;
	...
	//	Maximum multiplexed packets allowed (client can send up to this many SMBs before waiting for a response)
	//
  ...
  private String m_targetNBName;
	...
	//	Default SMB/CIFS flags and flags2, ORed with the SMB packet flags/flags2 before sending a response
	//	to the client.
	...
	//
	//	Contains SMB response packets that could not be sent due to SMB requests being processed. The asynchronous
	//	responses must be sent after any pending requests have been processed as the client may disconnect the
	//	session.
  ...
  /**
	...
	protected final void cleanupSession() {
	  ...
	  if (Debug.EnableInfo && hasDebug(DBG_STATE))
	    debugPrintln("Cleanup session, vcircuits=" + m_vcircuits.getCircuitCount() + ", changeNotify=" + getNotifyChangeCount());
   ...
   */
  protected void procNetBIOSSessionRequest()
   ...
   */
  protected void procSMBNegotiate()
    ...
    if (m_dialect == -1 || m_dialect <= Dialect.CorePlus) {
      ...
      m_smbPkt.setParameter(10, getServer().getGlobalConfiguration().getTimeZoneOffset());
      ...
      try {
        ...
        auth.generateNegotiateResponse( this, m_smbPkt, false);
      }
  ...
  public void run() {
    ...
    try {
      ...
      while (m_state != SMBSrvSessionState.NBHANGUP) {
				...
				//	Set the current receive length to -1 to indicate that the session thread is not currently processing
				//	an SMB packet. This is used by the asynchronous response code to determine when it can send the response.
        ...
        if (Debug.EnableInfo && hasDebug(DBG_RXDATA)) {
					debugPrintln("Rx Data len=" + m_rxlen);
					HexDump.Dump(m_buf, m_rxlen, 0, Debug.getDebugInterface());
        }
          ...
          case SMBSrvSessionState.SMBSESSION :
            ...
            runHandler();
						...
						if ( Debug.EnableInfo && hasDebug(DBG_TIMING)) {
							...
							if ( duration > 20)
						    debugPrintln("Processed packet " + PacketType.getCommandName(m_smbPkt.getCommand()) + " (0x" + Integer.toHexString(m_smbPkt.getCommand()) + ") in " + duration + "ms");
						}
            break;
   ...
   */
  protected final void runHandler()
    ...
    if (Debug.EnableInfo && hasDebug(DBG_PKTTYPE))
      debugPrintln("Rx packet type - " + m_smbPkt.getPacketTypeString() + ", SID=" + m_smbPkt.getSID());
    ...
    while ( hasAsynchResponse()) {
    	...
    	if ( Debug.EnableInfo && hasDebug(DBG_NOTIFY)) {
    		debugPrintln("Sent queued asynch response type=" + asynchPkt.getPacketTypeString() + ", mid=" + asynchPkt.getMultiplexId() + ", pid=" + asynchPkt.getProcessId());
	 ...
	 */
	public final void sendSuccessResponseSMB()
		throws IOException {
    ...
    //  Return the success response to the client
		...
		if ( Debug.EnableInfo && hasDebug(DBG_TXDATA))
			debugPrintln("Tx Data len=" + m_smbPkt.getLength() + ", success SMB");
	}
	
	/**
	 * Send an error response SMB. The returned code depends on the client long error code flag setting.
	 * 
  ...
  public final void sendErrorResponseSMB(int errCode, int errClass)
    ...
    if (Debug.EnableInfo && hasDebug(DBG_ERROR))
      debugPrintln("Error : Cmd = " + m_smbPkt.getPacketTypeString() + " - " + SMBErrorText.ErrorString(errClass, errCode));
  }
