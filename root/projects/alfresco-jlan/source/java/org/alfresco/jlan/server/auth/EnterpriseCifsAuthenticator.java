...
public class EnterpriseCifsAuthenticator extends CifsAuthenticator implements CallbackHandler
{
    ...
    throws SMBSrvException
  {
    ...
    if (Debug.EnableInfo && hasDebug())
      Debug.println("[SMB] NT Session setup " + (useRawNTLMSSP() ? "NTLMSSP" : "SPNEGO") + ", MID=" + reqPkt.getMultiplexId() + ", UID=" + reqPkt.getUserId() + ", PID=" + reqPkt.getProcessId());
  ...
  private final byte[] doNtlmsspSessionSetup( SMBSrvSession sess, ClientInfo client,
          byte[] secbuf, int secpos, int seclen, boolean unicode)
    throws SMBSrvException {
