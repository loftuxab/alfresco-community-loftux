 ...
 * FTPSrvSession.java
 *
 * Copyright (c) 2004-2006 Starlasoft. All rights reserved.
 */
...
public class FTPSrvSession extends SrvSession implements Runnable {
  ...
  private int m_mlstFacts = MLST_DEFAULT;
	...
	public final void closeSession() {
		...
		//	Close the socket first, if the client is still connected this should allow the input/output streams
		//	to be closed
  ...
  public final boolean isUTF8Enabled() {
    ...
    //	Convert the path to an FTP format path
		...
		//	Check if the path is relative
			...
			//	Check for the special '..' directory, if already at the root directory return an error
			...
			if ( ftpPath.hasSharedDevice() == false &&
					 ftpPath.setSharedDevice(getShareList(), this) == false)
				return null;
		}
  ...
  protected final void procOptions(FTPRequest req)
    ...
    if ( ftpPath.isRootPath() || ftpPath.isRootSharePath())
    {
		...
		if ( Debug.EnableInfo && hasDebug(DBG_FILE))
			debugPrintln("Returning ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", path=" + ftpPath.getSharePath());
   ...
   * @param append boolean
	...
	protected final void procStoreFile(FTPRequest req, boolean append)
      ...
      if ( sts == FileStatus.FileExists)
        openAction = append == false ? FileAction.TruncateExisting : FileAction.OpenIfExists;
			...
			if ( Debug.EnableInfo && hasDebug(DBG_FILE))
				debugPrintln("Storing ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", path=" + ftpPath.getSharePath() + (append ? " (Append)" : ""));
	...
	protected final void procDeleteFile(FTPRequest req)
		...
		try {
			...
			if ( sts == FileStatus.FileExists) {
				...
				if ( Debug.EnableInfo && hasDebug(DBG_FILE))
					debugPrintln("Deleted ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", path=" + ftpPath.getSharePath());
			}
			else {
	...
	protected final void procRenameFrom(FTPRequest req)
		...
		try {
			...
			if ( sts != FileStatus.NotExist) {
				...
				if ( Debug.EnableInfo && hasDebug(DBG_FILE))
					debugPrintln("RenameFrom ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", path=" + ftpPath.getSharePath());
			}
			else {
	...
	protected final void procRenameTo(FTPRequest req)
		...
		try {
			...
			if ( sts == FileStatus.NotExist) {
				...
				if ( Debug.EnableInfo && hasDebug(DBG_FILE))
					debugPrintln("RenameTo ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", path=" + ftpPath.getSharePath());
			}
			else {
	...
	protected final void procCreateDirectory(FTPRequest req)
		...
		try {
			...
			if ( sts == FileStatus.NotExist) {
				...
				FileOpenParams params = new FileOpenParams(ftpPath.getSharePath(), FileAction.CreateNotExist, AccessMode.ReadWrite,
				...
				if ( Debug.EnableInfo && hasDebug(DBG_DIRECTORY))
					debugPrintln("CreateDir ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", path=" + ftpPath.getSharePath());
			}
			else {
	...
	protected final void procRemoveDirectory(FTPRequest req)
		...
		//	Check if the path is the root directory, cannot delete directories from the root directory
		...
		try {
			...
			if ( sts == FileStatus.DirectoryExists) {
				...
				if ( Debug.EnableInfo && hasDebug(DBG_DIRECTORY))
					debugPrintln("DeleteDir ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", path=" + ftpPath.getSharePath());
			}
			else {
  ...
  protected final void procMachineListingContents(FTPRequest req)
    ...
    //  Check the format of the argument to detemine if this is a get or set modify date/time request
    //
    ...
    try {
      ...
      if ( Debug.EnableInfo && hasDebug(DBG_FILE))
        debugPrintln("File modify date/time ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", modified=" + finfo.getModifyDateTime());
    }
    catch (Exception ex) {
  ...
  protected final void procFeatures(FTPRequest req)
	...
	protected final void procFileSize(FTPRequest req)
		...
		try {
			...
			if ( Debug.EnableInfo && hasDebug(DBG_FILE))
				debugPrintln("File size ftp=" + ftpPath.getFTPPath() + ", share=" + ftpPath.getShareName() + ", size=" + finfo.getSize());
		}
		catch (Exception ex) {
  ...
  /**
   * Process an abort command. The main abort processing is done in the store/return file handling during
