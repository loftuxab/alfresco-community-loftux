...
public class EditRecordMetadataCapability extends AbstractCapability
{
    ...
    public EditRecordMetadataCapability(RMEntryVoter voter)
    {
        super(voter);
    }
    ...
    public int evaluate(NodeRef nodeRef)
    {
        if (isRm(nodeRef))
        {
            if (checkFilingUnfrozenUncutoffUndeclared(nodeRef) == AccessDecisionVoter.ACCESS_GRANTED)
            {
                if (isRecord(nodeRef))
                {
                    if (voter.getPermissionService().hasPermission(nodeRef, RMPermissionModel.EDIT_RECORD_METADATA) == AccessStatus.ALLOWED)
                    {
