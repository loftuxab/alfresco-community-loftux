...
public class DeclareRecordsInClosedFoldersCapability extends AbstractCapability
{
    ...
    public DeclareRecordsInClosedFoldersCapability(RMEntryVoter voter)
    {
        super(voter);
    }
    ...
    public int evaluate(NodeRef nodeRef)
    {
        if (isRm(nodeRef))
        {
            if (checkFilingUnfrozenUncutoff(nodeRef) == AccessDecisionVoter.ACCESS_GRANTED)
            {
                ...
                {
                    if (voter.getPermissionService().hasPermission(nodeRef, RMPermissionModel.DECLARE_RECORDS_IN_CLOSED_FOLDERS) == AccessStatus.ALLOWED)
                    {
