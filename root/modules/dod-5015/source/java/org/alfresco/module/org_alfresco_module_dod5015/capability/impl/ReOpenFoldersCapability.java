...
public class ReOpenFoldersCapability extends AbstractCapability
{
    ...
    // TODO: closed?
    public ReOpenFoldersCapability(RMEntryVoter voter)
    {
        super(voter);
    }
    ...
    public int evaluate(NodeRef nodeRef)
    {
        if (isRm(nodeRef))
        {
            if (checkFilingUnfrozenUncutoff(nodeRef) == AccessDecisionVoter.ACCESS_GRANTED)
            {
                ...
                {
                    if (voter.getPermissionService().hasPermission(nodeRef, RMPermissionModel.RE_OPEN_FOLDERS) == AccessStatus.ALLOWED)
                    {
