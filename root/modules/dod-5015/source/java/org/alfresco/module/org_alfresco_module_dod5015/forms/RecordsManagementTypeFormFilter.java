...
public class RecordsManagementTypeFormFilter extends RecordsManagementFormFilter
{
    ...
    {
        // we know the persisted object is a NodeRef so cast
        NodeRef nodeRef = (NodeRef)persistedObject;
        
        // Once an RM container type has been persisted generate a default identifer for it.
        if (this.nodeService.hasAspect(nodeRef, ASPECT_FILE_PLAN_COMPONENT))
        {
            ...
            this.nodeService.setProperty(nodeRef, RecordsManagementModel.PROP_IDENTIFIER,
                        generateIdentifier(nodeRef));
        }
    }
