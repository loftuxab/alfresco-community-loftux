...
{	
	...
	private static final String QUERY_SECTION_WITH_CHILDREN = "select " + COLUMNS + ", o.ws:orderIndex as ord "+
															    "from ws:section as f " +
																"join cm:titled as t on t.cmis:objectId = f.cmis:objectId "+
																"join ws:ordered as o on o.cmis:objectId = f.cmis:objectId "+
																"where (in_tree(f, {0}) or f.cmis:objectId = {1}) "+
																"order by ord";
	/*
	private static final String QUERY_COLLECTION_FOLDERS = "select f.cmis:objectId, f.cmis:parentId "+
	 ...
	 * be too. (Note: All sections for a website will only be present when rootSectionsByWebsite
	 ...
	 * 
	 * @param assetFactory	asset factory
	 */
	...
	private SectionDetails buildSection(QueryResult result) 
	{
        ...
        properties.put(Section.PROPERTY_DESCRIPTION, (String)result.getPropertyValueById(Section.PROPERTY_DESCRIPTION));
        properties.put(Section.PROPERTY_EXCLUDE_FROM_NAV, (Boolean)result.getPropertyValueById(Section.PROPERTY_EXCLUDE_FROM_NAV));
		...
		{ 
			...
			// TODO keep parent id in SectionDetails too as not accessible from resource. Is this deliberate?
	...
	/**
	 * Parses the section configuration from the name value pair string list into a map.
	 ...
	 * @param tagCounts  list of tag counts
	...
	/**
	 * @see org.alfresco.wcm.client.SectionFactory#getSectionFromPathSegments(String, String[])
	 */
	@Override
	...
	{	
		...
		{
		    if (segment.length() > 0) currentSection = currentSection.getSection(segment);
	 ...
	 * @param rootSectionId the id of the parent web root
	 */
	 ...
	 * @param topSectionId the section id to start from
	...
	private Section findSectionWithChildren(String topSectionId) {
		...
		// Fetch sections			
		String query = MessageFormat.format(QUERY_SECTION_WITH_CHILDREN, 
		...
		for (QueryResult result : results) 
		{
			...
			if ( ! sectionDetails.objectTypeId.equals("F:ws:webroot") 
				&& ! sectionDetails.objectTypeId.equals("F:ws:section")) continue;   
		...
		/* Not needed for webscript collection solution
		String cquery = MessageFormat.format(QUERY_COLLECTION_FOLDERS, 
											 SqlUtils.encloseSQLString(topSectionId));
		ItemIterable<QueryResult> cresults = session.query(cquery, false);

		for (QueryResult cresult : cresults) 
		{
			String collectionsFolderId = (String)cresult.getPropertyValueById(PropertyIds.OBJECT_ID);
			String sectionId = (String)cresult.getPropertyValueById(PropertyIds.PARENT_ID);
