 ...
 * 
 * DwsMetaData class contains more information about a Document Workspace site 
 ...
 * lists of a Document Workspace site.
 * 
 * @author AndreyAk
 *
 */
public class DwsMetadata implements Serializable
{
    ...
    //data returned by the GetDwsData method
    ...
    public String toString()
    {
        ...
        result.append(SoapUtils.startTag("Results"))
              .append(SoapUtils.proccesTag("SubscribeUrl", subscribeUrl))
              .append(SoapUtils.proccesTag("MtgInstance", mtgInstance))
              .append(SoapUtils.proccesTag("SettingUrl", settingsUrl))
              .append(SoapUtils.proccesTag("PermsUrl", permsUrl))
              .append(SoapUtils.proccesTag("UserInfoUrl", userInfoUrl))
              .append(SoapUtils.startTag("Roles"));
        for (String role : roles)
        {
            result.append("<Role Name=\"" + role + "\" Description=\"\" Type=\"" + role + "\" />");
        }
        result.append(SoapUtils.endTag("Roles"));
        if(!minimal)
        {            
            for (SchemaBean schema : schemaItems)
            {
                result.append(schema);
            }
            for (ListInfoBean listInfo : listInfoItems)
            {                
                result.append(listInfo);
            }              
        }
        result.append(SoapUtils.startTag("Permissions"));
        for (Permission permission : permissions)
        {
            result.append(SoapUtils.singleTag(permission.toString()));
        }
        result.append(SoapUtils.endTag("Permissions"))
              .append(SoapUtils.proccesTag("HasUniquePerm", hasUniquePerm))
              .append(SoapUtils.proccesTag("WorkspaceType", workspaceType))
              .append(SoapUtils.proccesTag("IsADMode", isADMode))
              .append(SoapUtils.proccesTag("DocUrl", docUrl))
              .append(SoapUtils.proccesTag("Minimal", minimal))
              .append(dwsData)
              .append(SoapUtils.endTag("Results"));
