package com.activiti.web.config;

import java.util.ArrayList;
import java.util.List;

/**
 * Default implementation of a config section
 * 
 * @author gavinc
 */
public class ConfigSectionImpl implements ConfigSection
{
   private String evaluator;
   private String condition;
   private List configElements;
   
   public ConfigSectionImpl(String evaluator, String condition)
   {
      this.evaluator = evaluator;
      this.condition = condition;
      this.configElements = new ArrayList();
   }
   
   /**
    * @see com.activiti.web.config.ConfigSection#getEvaluator()
    */
   public String getEvaluator()
   {
      return this.evaluator;
   }

   /**
    * @see com.activiti.web.config.ConfigSection#getCondition()
    */
   public String getCondition()
   {
      return this.condition;
   }

   /**
    * @see com.activiti.web.config.ConfigSection#getConfigElements()
    */
   public List getConfigElements()
   {
      return this.configElements;
   }

   /**
    * Adds a config element to the results for the lookup
    * 
    * @param configElement
    */
   public void addConfigElement(ConfigElement configElement)
   {
      this.configElements.add(configElement);
   }

   /**
    * @see com.activiti.web.config.ConfigSection#isGlobal()
    */
   public boolean isGlobal()
   {
      boolean global = false;
      
      if (this.evaluator == null || this.evaluator.length() == 0 ||
          this.condition == null || this.condition.length() == 0 )
      {
         global = true;
      }
      
      return global;
   }

   /**
    * @see java.lang.Object#toString()
    */
   public String toString()
   {
      StringBuilder buffer = new StringBuilder(super.toString());
      buffer.append(" (evaluator=").append(this.evaluator);
      buffer.append(" condition=").append(this.condition).append(")");
      return buffer.toString();
   }
   
}
