...
/**
 * This is the webscript controller for the invalid-domain.post webscript.
 * 
 * @author Neil Mc Erlean
 * @since Thor Phase 2 Sprint 1
 */
public class InvalidDomainPost extends AbstractInvalidDomainWebscript
{
    @Override protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache)
    {
        ...
        try
        {
            emailAddressService.createInvalidDomain(postedData.get(PARAM_DOMAIN), type, postedData.get(PARAM_NOTES));
        } catch (InvalidDomainException ide)
        {
            throw new WebScriptException(Status.STATUS_FORBIDDEN, "Cannot create invalid domain entry. Entry exists.", ide);
        }
