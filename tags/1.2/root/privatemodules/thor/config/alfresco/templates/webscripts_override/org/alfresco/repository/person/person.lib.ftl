...
<#local homeTenant=people.getHomeTenant(p.userName)!"isPublicHomeTenant">
<#escape x as jsonUtils.encodeJSONString(x)>
	"url": "${url.serviceContext + "/api/person/" + p.userName}",
	"userName": "${p.userName}",
	"homeTenant": <#if homeTenant != "isPublicHomeTenant">"${homeTenant}"<#else>null</#if>,
	...
	<#if (people.isFullProfileVisible(person.properties.userName) == true)>,
