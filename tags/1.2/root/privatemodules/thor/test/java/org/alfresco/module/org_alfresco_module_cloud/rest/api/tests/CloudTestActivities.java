...
public class CloudTestActivities extends CloudTestApi
{
	...
	{
		this.network1 = repoService.new TestNetwork("activitiesNetwork1", true);
		this.network2 = repoService.new TestNetwork("activitiesNetwork2", true);
        	...
        	public TestSite execute() throws Throwable
        	{
				return TenantUtil.runAsUserTenant(new TenantRunAsWork<TestSite>()
				{
					public TestSite doWork() throws Exception
					{
						...
						site.inviteToSite(person3.getId(), SiteRole.SiteCollaborator);
						return site;
					}
...
//		}
	...
	@Test
	public void testPUBLICAPI71() throws Exception
	{
		People peopleProxy = publicApiClient.people();

		// Test that a user sees only those activities from the network defined in the url
		{
			List<Activity> expectedActivities = TenantUtil.runAsUserTenant(new TenantRunAsWork<List<Activity>>()
			{
				@Override
				public List<Activity> doWork() throws Exception
				{
					List<Activity> activities = repoService.getActivities(person3.getId(), null, false, true);
					return activities;
				}
			}, person3.getId(), network1.getId());
			...
			Map<String, String> params = createParams(paging, null);
			params.put("who", String.valueOf(ActivityWho.me));
			ListResponse<Activity> activities = peopleProxy.getActivities("-me-", params);
			checkList(expectedActivities.subList(skipCount, skipCount + paging.getExpectedPaging().getCount()), paging.getExpectedPaging(), activities);
		}
		
		{
			...
			publicApiClient.setRequestContext(new RequestContext(network2.getId(), person3.getId()));
			...
			// -default- network is the user's home network
			publicApiClient.setRequestContext(new RequestContext("-default-", person3.getId()));

			skipCount = 0;
			maxItems = 10;
			paging = getPaging(skipCount, maxItems, expectedActivities.size(), null);
			params = createParams(paging, null);
			params.put("who", String.valueOf(ActivityWho.me));
			activities = peopleProxy.getActivities("-me-", params);
			checkList(expectedActivities.subList(skipCount, skipCount + paging.getExpectedPaging().getCount()), paging.getExpectedPaging(), activities);
		}
	}
}
