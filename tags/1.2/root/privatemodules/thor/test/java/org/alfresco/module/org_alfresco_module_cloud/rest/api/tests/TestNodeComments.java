...
public class TestNodeComments extends CloudTestApi
{
	@Test
	...
	public void testNodeComments() throws Exception
	{
		...
		final TestPerson person5 = people.get(4);
		...
		TenantUtil.runAsUserTenant(new TenantRunAsWork<Void>()
		{
			...
			public Void doWork() throws Exception
			{
                network1.addExternalUser(person4.getId()); // external user of network1
                network1.addExternalUser(person5.getId()); // external user of network1
				site1.inviteToSite(person4.getId(), SiteRole.SiteContributor);
		...
		final NodeRef folderNodeRef1 = nodes.get(1);
		final NodeRef nodeRef2 = nodes.get(2);
		final NodeRef nodeRef3 = nodes.get(3);
		final NodeRef nodeRef4 = nodes.get(4);
...

		{
			...
			for(Comment comment : comments)
			{
				...
				person1.expected(resp.getCreatedBy());
		...
		// person from a different network, member of the site
		try
		{
		...
		// person from a different network, not a member of the site, member of the network
		try
		{
			publicApiClient.setRequestContext(new RequestContext(network1.getId(), person5.getId()));
			Comment comment = expectedComments.get(1);
			Comment update = new Comment("Test Comment 4", "Test Comment 4");
			commentsProxy.updateNodeComment(nodeRef1.getId(), comment.getId(), update);
			fail("");
		...
		// successful update
		{
			...
			expected.setModifiedBy(repoService.getPerson(person1.getId()));
