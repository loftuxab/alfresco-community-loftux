...
<BODY topmargin=5 leftmargin=5 scroll=no serverType=OWS onload="checkScroll()" onresize="checkScroll()">
   ...
   <table ID="FileDialogViewTable" width="100%" class="recordSet" style="cursor: default;" cellspacing=0>
      ...
      <tr>
         <th style="padding: 2px; text-align: left" class="recordSetHeader">${type}</th>
         <th style="padding: 2px; text-align: left" class="recordSetHeader">${name}</th>
         <th style="padding: 2px; text-align: left" class="recordSetHeader">${modified_by}</th>
         <th style="padding: 2px; text-align: left" class="recordSetHeader">${modified}</th>
         <th style="padding: 2px; text-align: left" class="recordSetHeader">${checked_out_by}</th>
      </tr>
   ...
   <#list items as item>
      <#if item.isFolder()>
         <tr class="recordSetRow"  fileattribute=folder ID="${item.getUrl()}" onmousedown="selectrow('${item.getUrl()}')">
            <td style="padding: 2px; text-align: left"><IMG BORDER=0 ALT="Icon" SRC="${resourcesUrl}/images/icons/space-icon-default-16.gif"></td>
      ...
      <#else>
         <tr class="recordSetRow" fileattribute=file ID="${item.getUrl()}" onmousedown="selectrow('${item.getUrl()}')">
            <td style="padding: 2px; text-align: left"><IMG BORDER=0 ALT="Icon" SRC="${resourcesUrl}/${item.getFileTypeImage()}"></td>
            <td style="text-align: left">${item.getName()}</td>
            <td style="text-align: left">${item.getModifiedBy()}</td>
            <td style="text-align: left">${item.getModifiedTime()}</td>
