...
public class OLEServerPropertiesExtractionPostProcessor implements ContentPostProcessor
{
    ...
    protected static Logger logger = Logger.getLogger(OLEServerPropertiesExtractionPostProcessor.class);
    ...
    public void execute(NodeRef nodeRef)
    {
        try
        {
            ...
            for(Entry<QName, FieldDefinition> mapping : propertyMapping.entrySet())
            {
                ...
                Object afterValue = after.get(fieldDef.getDisplayName());
                if(! ((afterValue==null)||(afterValue instanceof String)))
                {
                    continue;
                }
    ...
    protected Serializable convertValue(String formattedNewValue, FieldDefinition fieldDef, QName alfrescoName) throws ParseException
    {
        ...
        switch (fieldType)
        {
            ...
            case Number:
            {
                if( (formattedNewValue.indexOf('.') < 0) && (formattedNewValue.indexOf(',') >= 0) )
                {
