...
SELECT alf_locale_seq.nextval, 0, string_value
FROM
(
...
USING
(
   SELECT tmp_locale_upgrade.node_id AS nid, alf_locale.id AS lid FROM tmp_locale_upgrade
   JOIN alf_locale ON (tmp_locale_upgrade.string_value = alf_locale.locale_str)
)
ON (alf_node.id = nid)
WHEN MATCHED THEN
UPDATE SET locale_id = lid;
