...
--ASSIGN:next_node_id=next_node_id
select case when max(id) is not null then max(id)+1 else 1 end as next_node_id from alf_node;
...
rename alf_node to t_alf_node;


create table alf_node
(
    id BIGINT not NULL GENERATED by default as IDENTITY (start with ${next_node_id}),
